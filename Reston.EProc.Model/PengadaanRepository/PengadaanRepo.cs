    ...
    public class PengadaanRepo : IPengadaanRepo
    {
        ...
        public VWCountListDokumen ListCount()
        {
            return new VWCountListDokumen() {
                ...
                //MonitorSelection = ctx.Pengadaans.Where(d => d.Status == EStatusPengadaan.PEMENANG && d.DokumenPengadaans.Where(dd => dd.Tipe == TipeBerkas.SuratPerintahKerja && dd.PengadaanId == d.Id).Count() > 0 && d.PersetujuanPemenangs.Count() > 0).Count() && d.MonitoringPekerjaans.Where(x => x.StatusMonitoring != StatusMonitored.TIDAK).Count() == 0) && d.MonitoringPekerjaans.Where(x => x.StatusSeleksi == StatusSeleksi.BELUMDITENTUKAN).Count() > 0).Count()
            };
        }
